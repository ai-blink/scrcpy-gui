...
![구성] 좌측 메뉴(화면/창/전원/입력/소리/녹화/기타/설정) + 실행될 명령어 미리보기 + 프리셋
...
| scrcpy | 아무 버전 | Android 16 폰이면 **4.0 이상 필요** (그 미만은 크래시) |
...
못 찾으면 창은 그대로 뜨고 하단에 안내가 나옵니다 → **왼쪽 `설정` 탭 → `찾아보기`** 로 지정하면
`scrcpy-gui-config.json`에 저장돼서 다음부터 기억합니다.
...
| 파일 | 무엇 | 언제 |
|---|---|---|
| `scrcpy-gui-config.json` | scrcpy·adb 실행 파일 위치 | `설정` 탭에서 경로를 바꿀 때 |
...
| `combo` | 목록에서 고르기 |
| `editcombo` | 목록 + 직접 입력 |
| `phonepref` | 폰 설정을 adb로 즉시 바꾸는 토글 (scrcpy 명령어에는 안 들어감) |
| `path` | 실행 파일 경로 + 찾아보기 |
